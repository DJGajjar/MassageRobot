...
class CreateRoutineViewController: UIViewController {
    ...
    func setRoutingDataServiceCall() {
        ...
        let url = "https://massage-robotics-website.uc.r.appspot.com/rd?query='Select * from Routine where routineId='\(strRoutingID!)''"
            ...
            if json.getString(key: "status") == "false"
            {
                ...
                do {
                    if let jsonArray = try JSONSerialization.jsonObject(with: data, options : .allowFragments) as? [Dictionary<String,Any>] {
                        ...
                        txtRoutineName.text = routingData.getString(key: "routinename")
                        txtType.text = routingData.getString(key: "routine_type")
                        txtUser.text = routingData.getString(key: "user_type")
                        txtCategory.text = routingData.getString(key: "routine_category")
                        txtDescription.text = routingData.getString(key: "description")
                        
                        let strLikeCount: String = String(format: "%@"," (" + routingData.getString(key: "likecount") + ")") 

                        btnLike.setTitle(strLikeCount, for: .normal)
                        
                        if routingData.getString(key: "thumbnail") != "" {
                            let urls = URL.init(string: routingData.getString(key: "thumbnail"))
                            imgRoutine.sd_setImage(with: urls , placeholderImage: UIImage(named: "DemoCellImg"))
                        }else {
                            imgRoutine.image = UIImage(named: "DemoCellImg")
                        }
